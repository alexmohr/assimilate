...
describe('LoginView', () => {
  ...
  it('passes rememberMe true when checkbox is checked', async () => {
    ...
    expect(authStore.login).toHaveBeenCalledWith('admin', 'password123', true)
  })
  ...
  it('calls verifyTotp when submitting TOTP code', async () => {
    ...
    expect(authStore.verifyTotp).toHaveBeenCalledWith('123456', false)
  })

  it('toggles to recovery-code input and submits with the recovery flag set', async () => {
    const wrapper = renderWithPlugins(LoginView)
    const authStore = useAuthStore()
    ...
    const toggleButton = wrapper
      .findAll('button')
      .find((b) => b.text().includes('Use a recovery code instead'))
    expect(toggleButton).toBeDefined()
    await toggleButton!.trigger('click')

    expect(wrapper.find('#totp-code').exists()).toBe(false)
    const recoveryInput = wrapper.find('#totp-recovery-code')
    expect(recoveryInput.exists()).toBe(true)

    await recoveryInput.setValue('aaaaaaaa-bbbbbbbb')
    await wrapper.find('form').trigger('submit.prevent')

    expect(authStore.verifyTotp).toHaveBeenCalledWith('aaaaaaaa-bbbbbbbb', true)
  })

  it('toggling back to the authenticator app clears the recovery code and shows the code input', async () => {
    const wrapper = renderWithPlugins(LoginView)
    const authStore = useAuthStore()
    ...
    const useRecoveryButton = wrapper
      .findAll('button')
      .find((b) => b.text().includes('Use a recovery code instead'))
    await useRecoveryButton!.trigger('click')
    await wrapper.find('#totp-recovery-code').setValue('aaaaaaaa-bbbbbbbb')

    const useAppButton = wrapper
      .findAll('button')
      .find((b) => b.text().includes('Use authenticator app instead'))
    expect(useAppButton).toBeDefined()
    await useAppButton!.trigger('click')

    expect(wrapper.find('#totp-recovery-code').exists()).toBe(false)
    const totpInput = wrapper.find<HTMLInputElement>('#totp-code')
    expect(totpInput.exists()).toBe(true)
    expect(totpInput.element.value).toBe('')
  })
  ...
  it('redirects to next path when a normal user logs in without password change', async () => {
    const routerPush = vi.fn()
    vi.mocked(useRouter).mockImplementationOnce(
      () => ({ push: routerPush }) as unknown as ReturnType<typeof useRouter>,
    )
