...
import { agentAvailabilityApi, getRepoAvailability, repoHostAvailabilityApi } from './availability'
...
describe('availability api', () => {
  ...
  it("loads a repository host's availability", async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: AVAILABILITY })

    await expect(repoHostAvailabilityApi(7).load()).resolves.toEqual(AVAILABILITY)

    expect(apiClient.get).toHaveBeenCalledWith('/repo-hosts/7/availability')
  })

  /** A repository only reads its host's settings; they are changed on the host. */
  it("reads a repository's view of its host's availability", async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: AVAILABILITY })

    await expect(getRepoAvailability(12)).resolves.toEqual(AVAILABILITY)

    expect(apiClient.get).toHaveBeenCalledWith('/repos/12/availability')
  })

  it("saves a repository host's availability with its re-check interval", async () => {
    ...
    await expect(repoHostAvailabilityApi(7).save(update)).resolves.toEqual(AVAILABILITY)

    expect(apiClient.put).toHaveBeenCalledWith('/repo-hosts/7/availability', update)
  })
  ...
  it('asks a repository host whether it is back', async () => {
    ...
    await expect(repoHostAvailabilityApi(7).check?.()).resolves.toEqual(outcome)

    expect(apiClient.post).toHaveBeenCalledWith('/repo-hosts/7/availability/check')
  })
  ...
  it('names the host its calls are bound to, domain included', () => {
    expect(repoHostAvailabilityApi(7).host).toBe('repo-host:7')
