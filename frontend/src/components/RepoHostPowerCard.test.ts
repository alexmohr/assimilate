...
import RepoHostPowerCard from './RepoHostPowerCard.vue'
...
function mount(props: Record<string, unknown> = {}) {
  return renderWithPlugins(RepoHostPowerCard, {
    props: { host: HOST, isAdmin: true, ...props },
  })
}

describe('RepoHostPowerCard', () => {
  beforeEach(() => {
    vi.mocked(apiClient.put).mockReset()
    vi.mocked(apiClient.put).mockResolvedValue({ data: HOST } as never)
  ...
  // `repo_hosts_shutdown_requires_mac` guarantees one exists.
  ...
  it('seeds every field from the current value when editing starts', async () => {
    ...
    expect(wrapper.find<HTMLInputElement>('#repo-host-power-wake-mac').element.value).toBe(
      '9C:B6:D0:1A:44:7F',
    )
    expect(wrapper.find<HTMLInputElement>('#repo-host-power-wake-broadcast').element.value).toBe(
      '192.168.1.255',
    )
    expect(wrapper.find<HTMLInputElement>('#repo-host-power-wake-timeout').element.value).toBe(
      '240',
    )
  })
  ...
  it('keeps the wake details on screen once the wake toggle is switched off', async () => {
    const wrapper = mount()
    await startEditingSection(wrapper)
    expect(wrapper.find('#repo-host-power-wake-mac').exists()).toBe(true)
  ...
  // the host with (`repo_hosts_shutdown_requires_mac`).
  it('resets shutdown-after-backup once the MAC address is cleared', async () => {
    const wrapper = mount()
    await startEditingSection(wrapper)
    await wrapper.find('#repo-host-power-wake-mac').setValue('')
    await flushPromises()
  ...
  it('sends the power settings object on save', async () => {
    const wrapper = mount()
    await startEditingSection(wrapper)
    await wrapper.find('#repo-host-power-wake-mac').setValue('AA:BB:CC:DD:EE:FF')
    await clickSectionButton(wrapper, 'Save')

    expect(apiClient.put).toHaveBeenCalledTimes(1)
    expect(apiClient.put).toHaveBeenCalledWith('/repo-hosts/42/power', {
      wake_enabled: true,
  ...
  it('sends edits to every field and the shutdown toggle on save', async () => {
    const wrapper = mount()
    await startEditingSection(wrapper)
    await wrapper.find('#repo-host-power-wake-broadcast').setValue('10.0.0.255')
    await wrapper.find('#repo-host-power-wake-timeout').setValue('300')
  ...
  it('stays in edit mode and shows the error when the save fails', async () => {
    ...
    const wrapper = mount()
    await expectSaveErrorKeepsEditing(wrapper, 'host offline', '#repo-host-power-wake-mac')
  })
  ...
  it('leaves the card without saving on Cancel', async () => {
    ...
    expect(wrapper.find('#repo-host-power-wake-mac').exists()).toBe(false)
    expect(apiClient.put).not.toHaveBeenCalled()
  })

  // See AgentPowerCard.test.ts: switching this host's own wake off no longer
  // means the host is never woken. Every repository on the host is asked, and
  // a schedule writing to two of them is named once.
