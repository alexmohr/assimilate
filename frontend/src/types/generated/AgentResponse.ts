// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { AgentPowerSettingsResponse } from "./AgentPowerSettingsResponse";
import type { HookCommand } from "./HookCommand";

/**
 * Response containing agent.
 */
export type AgentResponse = {
  /**
   * Unique identifier.
   */
  id: number;
  /**
   * Hostname of the machine.
   */
  hostname: string;
  /**
   * Human-readable display name.
   */
  display_name: string | null;
  /**
   * Version of the agent software.
   */
  agent_version: string | null;
  /**
   * agent git sha.
   */
  agent_git_sha: string | null;
  /**
   * agent build time.
   */
  agent_build_time: string | null;
  /**
   * agent commit count.
   */
  agent_commit_count: number | null;
  /**
   * Timestamp of when the created occurred.
   */
  created_at: string;
  /**
   * Timestamp of when the last seen occurred.
   */
  last_seen_at: string | null;
  /**
   * Default paths to include in backups.
   */
  default_backup_paths: Array<string>;
  /**
   * Default exclude patterns.
   */
  default_exclude_patterns: Array<string>;
  /**
   * Default commands to run before backups.
   */
  default_pre_backup_commands: Array<HookCommand>;
  /**
   * Default commands to run after backups.
   */
  default_post_backup_commands: Array<HookCommand>;
  /**
   * Default file change patterns.
   */
  default_file_change_patterns_raw: string;
  /**
   * Whether the agent is currently connected.
   */
  is_connected: boolean;
  /**
   * Whether this agent was imported from an external source.
   */
  is_imported: boolean;
  /**
   * Whether this agent is hidden from the UI.
   */
  is_hidden: boolean;
  /**
   * Whether the agent supports restarting.
   */
  supports_restart: boolean;
  /**
   * Identifier of the associated owner.
   */
  owner_id: number | null;
  /**
   * Visibility scope of this entity.
   */
  visibility: string;
  /**
   * Reason why restart is unavailable, if applicable.
   */
  restart_unavailable_reason: string | null;
  /**
   * SSH username last used to deploy/upgrade this agent.
   */
  last_ssh_user: string | null;
  /**
   * Optional DNS domain, set by an admin to disambiguate agents that
   * share an OS hostname across different networks.
   */
  domain: string | null;
  /**
   * Power-management settings: waking this agent's host before a backup,
   * and starting/stopping the agent process itself over SSH.
   */
  power: AgentPowerSettingsResponse;
};
