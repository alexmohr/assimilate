...
| `public_url` | unset | Base URL (scheme + host, e.g. `https://backups.example.com`) used to build absolute deep links -- such as an Activity Log link on a failed or warning backup -- in email and webhook notifications. Must use `http` or `https` and must be a bare origin -- no path, query, or fragment (a trailing slash is stripped automatically, but anything else is rejected). Unset by default, in which case those notifications omit the link. Web Push notifications don't need this setting: the browser resolves their links against its own origin. There is no dedicated field in the Settings UI yet -- set it via `PUT /api/system/settings` (see [Notifications](notifications.md#activity-log-deep-links)). |
