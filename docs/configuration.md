...
| `vm_snapshot_enabled` | When `true`, the libvirt/QEMU domains of every host this schedule targets are staged before the backup starts, using each host's own settings. See [VM Snapshots](vm-snapshots.md). |
