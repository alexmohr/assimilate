...
| `failed_report_retention_days` | `365` | Days to keep failed/archive-less backup reports. `0` = keep forever. To clear an agent's failed reports immediately rather than waiting on this window, use **Clean up failed backups** in its [header overflow menu](agents.md#logs-tab). |
...
| `vm_snapshot_enabled` | When `true`, the libvirt/QEMU domains of every host this schedule targets are staged before the backup starts, using each host's own settings. Hosts that do not allow it are skipped. See [VM Snapshots](vm-snapshots.md). |
...
| `catch_up_missed_runs` | When `true`, an occurrence missed because a target host or repository was unreachable is run once it is back — see [Catch-up runs](scheduling.md#catch-up-runs). Missed runs never stack: however many occurrences pass during an outage, at most one catch-up run follows. Defaults to `false`. |
| `catch_up_min_lead_minutes` | Minimum time, in minutes, that must remain before the next scheduled run for a catch-up to still start. A host or repository coming back closer than this to the next run has its pending miss dropped instead, so the catch-up never collides with the regular run. Between `1` and `10080` (7 days). Defaults to `120`. |
| `catch_up_repo_recheck_minutes` | How often, in minutes, a repository that was absent when the run came due is asked over SSH whether it is back — see [Waiting on a repository](scheduling.md#waiting-on-a-repository). Only the repository half of catch-up polls; an agent announces its own return. Between `1` and `10080` (7 days). Defaults to `15`. |
| `catch_up_give_up_minutes` | How long, in minutes, a pending catch-up may wait before it is abandoned and reported as a failed backup, measured from the occurrence it missed. `0` (the default) waits indefinitely. Any other value must be between `1` and `43200` (30 days) and at least one `catch_up_repo_recheck_minutes` interval. |
