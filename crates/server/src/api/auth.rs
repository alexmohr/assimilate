...
#[cfg(test)]
mod tests {
    use super::{CookieSecurity, helpers};
    ...
    async fn verify_password_runs_in_constant_time_with_dummy_hash() {
        // The dummy hash used in the login handler for nonexistent users must
        // be a valid bcrypt hash so that bcrypt runs in constant time and does
        // not leak whether the username exists via timing.
        let dummy_hash = "$2b$12$UPq1GccVoXUwuom5gyGexOmuF8evhCzdaIb.3EacmKJs8WODdyusC".to_string();

        // Any password must not match (false returned, not an error)
