...
- `MOCK_BORG_SIMULATE_IGNORED_ERROR=1` — exits `1` with an `ERROR` record as its only diagnostic, for the case where a file change pattern would otherwise suppress it.
