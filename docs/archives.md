...
Deleting an archive only unlinks it from the repository's manifest — the segment data it referenced is not reclaimed until the repository is compacted. Once the deletion queue for a repository has fully drained, Assimilate automatically runs `borg compact` on it to reclaim that space, shown as a **Compacting repository** indicator. Deleting several archives back to back only triggers one compact after the last one finishes, not one per archive. If the compact itself fails, the deletion still stands — the failure is logged as an `archive_compact_failed` system event and the freed space is reclaimed on the next opportunity (for example, a scheduled backup's own compact step).
