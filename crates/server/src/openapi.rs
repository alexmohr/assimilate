// SPDX-License-Identifier: Apache-2.0
// SPDX-FileCopyrightText: 2026 Alexander Mohr

use shared::responses::{
    AgentPowerSettingsResponse, AgentResponse, AgentTagEntryResponse, ArchiveEntryResponse,
    ArchiveIndexStatusResponse, ArchiveInfoResponse, AuditEntryResponse, AuditLogResponse,
    CalendarDayResponse, CalendarEventResponse, CreateAgentResponse, CreateApiTokenResponse,
    DashboardAgentLinkResponse, DashboardDestinationResponse, DashboardFindingResponse,
    DashboardOperationResponse, DashboardOverviewResponse, DashboardProtectionCoverageResponse,
    DashboardRepositoryCapacityResponse, DashboardSummaryCountersResponse,
    DashboardSummaryResponse, DashboardUpcomingScheduleResponse, DeleteApiTokenResponse,
    HealthCheckResponse, HealthResponse, HostWakeSettingsResponse, InitRepoResponse,
    ListApiTokensResponse, LogEntryResponse, LoginResponse, MeResponse, PassphraseResponse,
    RefreshSessionResponse, RepoQuotaResponse, RepoResponse, RepoTagEntryResponse,
    RepoWithStatsResponse, RescanResponse, RunEventResponse, ServerQuotaResponse,
    SessionListResponse, StorageRepoEntryResponse, StorageTrendByRepoEntryResponse,
    StorageTrendEntryResponse, TagResponse, Theme, TrendEntryResponse, UserPreferences,
};
use utoipa::OpenApi;

use crate::{
    api::{
        agents::{
            CreateAgentRequest, UpdateAgentPowerRequest, UpdateAgentRequest, UpdateHostWakeRequest,
        },
        archives::{ContentEntry, ContentsResponse},
        audit::AuditLogQuery,
        auth::{ChangePasswordRequest, LoginRequest, RefreshResponse},
        config_io::{ConfigExport, HostExport, ImportResult, ScheduleExport, ScheduleTargetExport},
        deploy::{DeployAgentRequest, DeployAgentResponse},
        diff::DiffResponse,
        dryrun::{DryRunFileEntry, DryRunRequest, DryRunResponse},
        excludes::SetGlobalExcludesRequest,
        logs::LogQuery,
        permissions::UpsertPermissionRequest,
        quota::UpsertQuotaRequest,
        repos::{CreateRepoRequest, InitRepoRequest, UpdateRepoRequest},
        schedules::{CreateScheduleRequest, RunScheduleRequest, UpdateScheduleRequest},
        search::{SearchEntry, SearchResponse},
        server_quotas::UpsertServerQuotaRequest,
        system::{
            DatabaseStorageResponse, SettingsResponse, SshPublicKeyResponse, SystemResetResponse,
            UpdateSettingsRequest, VersionResponse,
        },
        tags::{CreateTagRequest, SetTagsRequest},
        tokens::CreateTokenRequest,
        users::{CreateUserRequest, UpdatePasswordRequest},
    },
    db::{
        ActivityRow, AgentRow, ApiTokenRow, GlobalExcludesConfig, RepoPermissionRow, ReportRow,
        ScheduleCountByAgent, ScheduleRow, ScheduleTargetRow, StorageStatRow, SystemEventRow,
    },
    ssh::{
        DeployKeyRequest, DeployKeyResponse, DirEntryInfo, ListDirRequest, ListDirResponse,
        TestConnectionRequest, TestConnectionResponse,
    },
};

#[derive(OpenApi)]
#[openapi(
    info(title = "Assimilate API", version = "1.0.0"),
    paths(
        crate::api::auth::login,
        crate::api::auth::logout,
        crate::api::auth::me,
        crate::api::auth::refresh_session,
        crate::api::auth::change_password,
        crate::api::auth::get_preferences,
        crate::api::auth::update_preferences,
        crate::api::totp::totp_setup,
        crate::api::totp::totp_verify,
        crate::api::totp::totp_verify_login,
        crate::api::totp::totp_disable,
        crate::api::totp::totp_recovery,
        crate::api::auth::list_sessions,
        crate::api::auth::revoke_session,
        crate::api::users::list_users,
        crate::api::users::create_user,
        crate::api::users::update_password,
        crate::api::users::delete_user,
        crate::api::tokens::create_token,
        crate::api::tokens::list_tokens,
        crate::api::tokens::delete_token,
        crate::api::agents::list_agents,
        crate::api::agents::create_agent,
        crate::api::agents::get_agent,
        crate::api::agents::update_agent,
        crate::api::agents::delete_agent,
        crate::api::agents::regenerate_token,
        crate::api::agents::restart_agent,
        crate::api::agents::update_agent_power,
        crate::api::vms::get_agent_vms,
        crate::api::vms::update_agent_vm_snapshot,
        crate::api::vms::update_agent_vm,
        crate::api::vms::scan_agent_vms,
        crate::api::vms::build_agent_vm,
        crate::api::vms::snapshot_agent_vm,
        crate::api::agents::cancel_agent_backup,
        crate::api::repos::list_repos,
        crate::api::repos::get_agent_repos,
        crate::api::repos::create_repo,
        crate::api::repos::update_repo,
        crate::api::repos::delete_repo,
        crate::api::repos::get_passphrase,
        crate::api::repos::list_repos_with_stats,
        crate::api::repos::get_repo,
        crate::api::repos::init_repo,
        crate::api::repos::rescan_repo,
        crate::api::repos::sync_repo,
        crate::api::repos::reset_import,
        crate::api::repos::list_schedules_for_repo,
        crate::api::archives::list_archives,
        crate::api::archives::archive_info,
        crate::api::archives::delete_archive,
        crate::api::archives::list_contents,
        crate::api::archives::get_archive_index_status,
        crate::api::archives::extract_file,
        crate::api::dryrun::dry_run,
        crate::api::search::search_archive,
        crate::api::tags::list_archive_tags,
        crate::api::tags::add_archive_tag,
        crate::api::tags::remove_archive_tag,
        crate::api::deploy::deploy_agent,
        crate::api::deploy::fetch_service_unit,
        crate::api::schedules::list_schedules,
        crate::api::schedules::create_schedule,
        crate::api::schedules::get_schedule,
        crate::api::schedules::update_schedule,
        crate::api::schedules::delete_schedule,
        crate::api::schedules::run_schedule_now,
        crate::api::schedules::cancel_running_backup,
        crate::api::schedules::list_schedule_reports,
        crate::api::schedules::delete_failed_schedule_reports,
        crate::api::schedules::count_failed_schedule_reports,
        crate::api::schedules::list_schedule_targets,
        crate::api::schedules::list_schedule_repos,
        crate::api::schedules::list_schedule_backup_sources,
        crate::api::schedules::list_schedule_catch_up_sources,
        crate::api::availability::get_repo_availability,
        crate::api::availability::get_repo_host_availability,
        crate::api::availability::update_repo_host_availability,
        crate::api::availability::check_repo_host_availability_now,
        crate::api::repo_hosts::list_repo_hosts,
        crate::api::repo_hosts::get_repo_host,
        crate::api::repo_hosts::update_repo_host,
        crate::api::repo_hosts::delete_repo_host,
        crate::api::repo_hosts::update_repo_host_power,
        crate::api::repo_hosts::scan_repo_host_key,
        crate::api::repo_hosts::accept_repo_host_key,
        crate::api::availability::get_agent_availability,
        crate::api::availability::update_agent_availability,
        crate::api::excludes::get_excludes,
        crate::api::excludes::set_excludes,
        crate::api::permissions::list_for_repo,
        crate::api::permissions::upsert,
        crate::api::permissions::list_for_user,
        crate::api::tags::list_tags,
        crate::api::tags::create_tag,
        crate::api::tags::delete_tag,
        crate::api::tags::set_repo_tags,
        crate::api::tags::get_repo_tags,
        crate::api::tags::set_agent_tags,
        crate::api::tags::get_agent_tags,
        crate::api::tags::list_agent_tag_associations,
        crate::api::tags::list_repo_tag_associations,
        crate::api::stats::summary,
        crate::api::stats::dashboard_overview,
        crate::api::stats::storage,
        crate::api::stats::storage_breakdown,
        crate::api::stats::activity,
        crate::api::stats::acknowledge_activity_entry,
        crate::api::stats::unacknowledge_activity_entry,
        crate::api::stats::acknowledge_all_activity,
        crate::api::stats::outstanding_acknowledgements,
        crate::api::stats::system_events,
        crate::api::stats::acknowledge_system_event,
        crate::api::stats::unacknowledge_system_event,
        crate::api::stats::health,
        crate::api::stats::trends,
        crate::api::stats::calendar,
        crate::api::stats::schedule_counts,
        crate::api::system::ssh_public_key,
        crate::api::system::ssh_regenerate_key,
        crate::api::system::get_settings,
        crate::api::system::update_settings,
        crate::api::system::get_database_storage,
        crate::api::system::get_version,
        crate::api::system::reset_system,
        crate::api::ssh::test_connection,
        crate::api::ssh::deploy_key,
        crate::api::ssh::list_dir,
        crate::api::health::health,
        crate::api::reports::list_reports,
        crate::api::reports::list_run_events,
        crate::api::reports::delete_failed_reports,
        crate::api::reports::count_failed_reports,
        crate::api::logs::get_logs,
        crate::api::keys::export_key,
        crate::api::keys::import_key,
        crate::api::keys::change_passphrase,
        crate::api::restore::download_files,
        crate::api::restore::restore_files,
        crate::api::server_quotas::list_server_quotas,
        crate::api::server_quotas::upsert_server_quota,
        crate::api::server_quotas::delete_server_quota,
        crate::api::audit::list_audit_log,
        crate::api::config_io::export_config,
        crate::api::config_io::import_config,
        crate::api::diff::diff_archives,
        crate::api::export::export_archive,
        crate::api::quota::get_quota,
        crate::api::quota::upsert_quota,
    ),
    components(schemas(
        LoginRequest,
        LoginResponse,
        MeResponse,
        RefreshResponse,
        ChangePasswordRequest,
        CreateUserRequest,
        UpdatePasswordRequest,
        CreateTokenRequest,
        CreateApiTokenResponse,
        ListApiTokensResponse,
        ApiTokenRow,
        AgentRow,
        AgentResponse,
        CreateAgentRequest,
        CreateAgentResponse,
        UpdateAgentRequest,
        UpdateAgentPowerRequest,
        UpdateHostWakeRequest,
        AgentPowerSettingsResponse,
        HostWakeSettingsResponse,
        RepoResponse,
        RepoWithStatsResponse,
        CreateRepoRequest,
        UpdateRepoRequest,
        RunEventResponse,
        PassphraseResponse,
        InitRepoRequest,
        InitRepoResponse,
        RescanResponse,
        ArchiveEntryResponse,
        ArchiveInfoResponse,
        ArchiveIndexStatusResponse,
        ContentEntry,
        ContentsResponse,
        DeployAgentRequest,
        DeployAgentResponse,
        CreateScheduleRequest,
        UpdateScheduleRequest,
        RunScheduleRequest,
        ScheduleRow,
        ScheduleTargetRow,
        DryRunRequest,
        DryRunFileEntry,
        DryRunResponse,
        SetGlobalExcludesRequest,
        GlobalExcludesConfig,
        UpsertPermissionRequest,
        RepoPermissionRow,
        CreateTagRequest,
        SetTagsRequest,
        TagResponse,
        AgentTagEntryResponse,
        RepoTagEntryResponse,
        DashboardSummaryResponse,
        DashboardOverviewResponse,
        DashboardSummaryCountersResponse,
        DashboardFindingResponse,
        DashboardDestinationResponse,
        DashboardProtectionCoverageResponse,
        DashboardAgentLinkResponse,
        DashboardOperationResponse,
        DashboardUpcomingScheduleResponse,
        shared::responses::AgentVmSnapshotResponse,
        shared::responses::AgentVmSnapshotSettingsResponse,
        shared::responses::AgentVmResponse,
        shared::vm::VmState,
        shared::vm::VmSnapshotMode,
        shared::vm::VmSelectionMode,
        crate::api::vms::UpdateAgentVmSnapshotRequest,
        crate::api::vms::UpdateAgentVmRequest,
        crate::api::vms::BuildAgentVmRequest,
        shared::vm::VmBuildAction,
        shared::vm::VmBuildOutcome,
        DashboardRepositoryCapacityResponse,
        StorageRepoEntryResponse,
        HealthResponse,
        TrendEntryResponse,
        StorageTrendEntryResponse,
        StorageTrendByRepoEntryResponse,
        CalendarDayResponse,
        CalendarEventResponse,
        crate::api::stats::TrendsQuery,
        crate::api::stats::CalendarQuery,
        StorageStatRow,
        ScheduleCountByAgent,
        ActivityRow,
        SystemEventRow,
        shared::types::SystemEventSeverity,
        shared::types::AcknowledgedFilter,
        crate::api::stats::AcknowledgementCountsResponse,
        SshPublicKeyResponse,
        SettingsResponse,
        UpdateSettingsRequest,
        DatabaseStorageResponse,
        crate::db::DatabaseRelationSizeRow,
        VersionResponse,
        SystemResetResponse,
        TestConnectionRequest,
        TestConnectionResponse,
        DeployKeyRequest,
        DeployKeyResponse,
        ListDirRequest,
        ListDirResponse,
        DirEntryInfo,
        ReportRow,
        LogQuery,
        SearchEntry,
        SearchResponse,
        LogEntryResponse,
        crate::api::keys::ImportKeyRequest,
        crate::api::keys::ChangePassphraseRequest,
        crate::api::restore::DownloadFilesRequest,
        crate::api::restore::RestoreFilesRequest,
        crate::api::restore::RestoreFilesResponse,
        HealthCheckResponse,
        DeleteApiTokenResponse,
        RefreshSessionResponse,
        UserPreferences,
        Theme,
        ServerQuotaResponse,
        UpsertServerQuotaRequest,
        AuditLogQuery,
        AuditLogResponse,
        AuditEntryResponse,
        shared::audit::AuditEvent,
        HostExport,
        ScheduleTargetExport,
        ScheduleExport,
        ConfigExport,
        ImportResult,
        DiffResponse,
        RepoQuotaResponse,
        UpsertQuotaRequest,
        shared::responses::TotpSetupResponse,
        shared::responses::TotpVerifyResponse,
        SessionListResponse,
        shared::responses::SessionResponse,
        crate::api::totp::TotpVerifyRequest,
        crate::api::totp::TotpDisableRequest,
        crate::api::totp::TotpRecoveryRequest,
        crate::api::totp::TotpLoginVerifyRequest,
    ))
)]
/// `OpenAPI` (`Swagger`) documentation struct generated by utoipa.
pub struct ApiDoc;

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn summaries_come_from_doc_comments() {
        let doc = ApiDoc::openapi();
        let json = serde_json::to_string(&doc).unwrap();
        assert!(json.contains("Remove a repository from the database"));
        assert!(json.contains("List audit log entries"));
        assert!(!json.contains("\"summary\":null"));
    }

    /// The hook command fields on `AgentRow` and `ScheduleRow` carry a
    /// `#[schema(value_type = ...)]` override, because the rows hold them as
    /// `sqlx::types::Json`, which utoipa cannot infer through. An override is
    /// a hand-written copy of the real type and goes stale silently: it kept
    /// claiming `Vec<String>` after hook commands became objects, so the
    /// published schema described a shape the API had stopped serving.
    #[test]
    fn hook_command_fields_are_documented_as_objects() {
        let doc = ApiDoc::openapi();
        let json = serde_json::to_value(&doc).unwrap();
        let schemas = json
            .get("components")
            .and_then(|components| components.get("schemas"))
            .expect("the spec declares component schemas");

        assert!(
            schemas.get("HookCommand").is_some(),
            "HookCommand must be a reachable component for the refs below to resolve"
        );

        for (schema, field) in [
            ("AgentRow", "default_pre_backup_commands"),
            ("AgentRow", "default_post_backup_commands"),
            ("ScheduleRow", "pre_backup_commands"),
            ("ScheduleRow", "post_backup_commands"),
        ] {
            let property = schemas
                .get(schema)
                .and_then(|row| row.get("properties"))
                .and_then(|properties| properties.get(field))
                .unwrap_or_else(|| panic!("{schema}.{field} is missing from the spec"));
            let item_ref = property
                .get("items")
                .and_then(|items| items.get("$ref"))
                .and_then(serde_json::Value::as_str);
            assert_eq!(
                item_ref,
                Some("#/components/schemas/HookCommand"),
                "{schema}.{field} must be an array of HookCommand, got {property}"
            );
        }
    }
}
