...
- **`/api/auth/login`** and **`/api/auth/totp/verify-login` + `/api/auth/totp/recovery`** (which share a bucket with each other, separate from plain login) are each capped at **30 requests per minute per IP**, as a coarser backstop on top of the per-account lockouts above.
